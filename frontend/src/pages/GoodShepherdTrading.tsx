...
import {
  fetchGoodShepherdCharacters,
  fetchTradingLocks,
  drawGoodShepherdStudents,
  fetchGoodShepherdCharacterMarkets,
  fetchGoodShepherdHouseMarkets,
  fetchGoodShepherdSpecialMarkets,
  settleGoodShepherdBets,
  endGoodShepherdSession
} from '../lib/api/api';
...
const Card: React.FC<{ character: Character | null; faceDown: boolean }> = ({ character, faceDown }) => {
  if (faceDown) {
    const faceDownImageUrl = `${API_BASE}/goodshepherd/facedown.png`;
    return (
  ...
  const imageUrl = `${API_BASE}/goodshepherd/${character.img_filename}`;
  ...
  return (
    <div className="card card-face-up">
      ...
      <div className="card-name">
        <h3>
          {character.name} <span style={{ fontFamily: 'monospace', fontWeight: '600' }}>({character.roll_number})</span>
        </h3>
      </div>
      ...
      <div className="card-stats">
        <div className="card-stat-row">
          <div className="card-stat">
            <div className="card-stat-label">DOB</div>
            <div className="card-stat-value" style={{ fontSize: '0.75rem' }}>
              {new Date(character.dob).toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}
            </div>
          </div>
          <div className="card-stat">
            <div className="card-stat-label">Height (cm)</div>
            <div className="card-stat-value">{character.height}</div>
          </div>
        </div>
        ...
        <div className="card-stat-row">
          <div className="card-stat">
            ...
            <div className="card-stat-value" style={{ fontSize: '0.8rem' }}>{character.house}</div>
          </div>
          <div className="card-stat">
            <div className="card-stat-label">Year Joined</div>
            <div className="card-stat-value">{character.year_joined}</div>
          </div>
        </div>

        <div className="card-stat-row">
          <div className="card-stat">
            <div className="card-stat-label">Expelled</div>
            <div className="card-stat-value">
              {character.expelled ? '✓' : '✗'}
            </div>
          </div>
          <div className="card-stat">
            <div className="card-stat-label">Prefect</div>
            <div className="card-stat-value">
              {character.was_prefect ? '✓' : '✗'}
            </div>
          </div>
        </div>
        ...
        <div className="card-stat-row">
          <div className="card-stat">
            <div className="card-stat-label">404</div>
            <div className="card-stat-value">
              {character.was_404 ? '✓' : '✗'}
            </div>
          </div>
          <div className="card-stat">
            <div className="card-stat-label">Sport</div>
            <div className="card-stat-value" style={{ fontSize: '0.8rem' }}>{character.sport || 'N/A'}</div>
          </div>
        </div>
      </div>
    </div>
...
export default function GoodShepherdTrading() {
  ...
  const [marketType, setMarketType] = useState<'houses' | 'sports'>('houses');
  ...
  const loadCharacters = async () => {
    try {
      const response = await fetchGoodShepherdCharacters();
  ...
  // Stats loading removed - not needed for Good Shepherd
  ...
  const startNewDraw = async () => {
    try {
      const drawResponse = await drawGoodShepherdStudents();
      if (drawResponse.success) {
        ...
        // Pass num_cards and draw_number to all market endpoints
        const charMarketsResponse = await fetchGoodShepherdCharacterMarkets(cardsCount);
        if (charMarketsResponse.success) setCharacterMarkets(charMarketsResponse.character_markets);

        const crewMarketsResponse = await fetchGoodShepherdHouseMarkets(cardsCount, drawNumber);
        if (crewMarketsResponse.success) {
          setCrewMarkets(crewMarketsResponse.crew_markets);
          setMarketType(crewMarketsResponse.market_type || 'houses');
        }

        const specialMarketsResponse = await fetchGoodShepherdSpecialMarkets(cardsCount);
  ...
  const handleSubmit = async (submittedBets?: Bet[]) => {
    ...
    try {
      const response = await settleGoodShepherdBets(cards, betsToSettle);
      ...
      <div className="sopranos-container">
        <div className="sopranos-content">
          <div style={{ marginBottom: '2rem' }}>
            ...
            <h1 className="sopranos-title">Good Shepherd Trading</h1>
              ...
              <div className="modal-content">
                ...
                <div className="rules-section">
                  ...
                  <ul>
                    ...
                    <li>Each draw shows 3 random students face-down</li>
                ...
                <div className="rules-section">
                  ...
                  <div style={{ marginBottom: '1rem', display: 'flex', gap: '2rem', fontSize: '1.1rem', fontWeight: '600' }}>
                    <div>Total Students: {characters.length}</div>
                    <div>Prefects: {characters.filter(c => c.was_prefect).length}</div>
                    <div>Expelled Students: {characters.filter(c => c.expelled).length}</div>
                  </div>
                  <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '2rem', fontSize: '1rem', fontWeight: '600' }}>
                    <div>Springsters: {characters.filter(c => c.house === 'Spring').length}</div>
                    <div>Summarians: {characters.filter(c => c.house === 'Summer').length}</div>
                    <div>Autumnites: {characters.filter(c => c.house === 'Autumn').length}</div>
                    <div>Wintarians: {characters.filter(c => c.house === 'Winter').length}</div>
                  </div>

                  <h4 style={{ marginTop: '1.5rem', marginBottom: '0.5rem', color: '#fbbf24' }}>All Students</h4>
                  <table className="stats-table">
                    ...
                    <tbody>
                      {characters.sort((a, b) => a.roll_number - b.roll_number).map(char => (
                        <tr key={char.character_id}>
                          <td>{char.name}</td>
                          <td style={{ textAlign: 'center' }}>{char.roll_number}</td>
                          <td>{char.house}</td>
                          <td style={{ textAlign: 'center' }}>{char.height}</td>
                          <td style={{ textAlign: 'center' }}>{char.was_prefect ? '✓' : '✗'}</td>
                          <td style={{ textAlign: 'center' }}>{char.expelled ? '✓' : '✗'}</td>
                          <td style={{ textAlign: 'center' }}>{char.was_404 ? '✓' : '✗'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>

                  {['Spring', 'Summer', 'Autumn', 'Winter'].map(houseName => {
                    const houseMembers = characters.filter(c => c.house === houseName);
                    if (houseMembers.length === 0) return null;
                    return (
                      <div key={houseName} style={{ marginBottom: '1rem' }}>
                        <h4 style={{ marginTop: '1.5rem', marginBottom: '0.5rem', color: '#fbbf24' }}>{houseName} House</h4>
                        <table className="stats-table">
                          <thead>
                            <tr>
                              <th>Name</th>
                          ...
                          <tbody>
                            {houseMembers.sort((a, b) => a.roll_number - b.roll_number).map(char => (
                              <tr key={char.character_id}>
                                <td>{char.name}</td>
                                <td style={{ textAlign: 'center' }}>{char.roll_number}</td>
                                <td style={{ textAlign: 'center' }}>{char.year_joined}</td>
                                <td style={{ textAlign: 'center' }}>{char.was_prefect ? '✓' : '✗'}</td>
                              </tr>
                            ))}
                          </tbody>
          ...
          <div className="cards-section">
            <div className="cards-panel">
              ...
              {cardsRevealed && showNextDrawButton && (
                <div className="submit-container">
                  <button onClick={() => { setDrawNumber(drawNumber + 1); startNewDraw(); }} className="btn-next-draw">
                    NEXT DRAW
                  </button>
            ...
            {/* Student Markets below cards - 2 columns */}
            <div className="character-markets-panel">
              <div className="character-markets-columns">
                ...
                {/* HOUSES/SPORTS DRAWN Column */}
                <div className="character-market-column">
                  <h3 className="character-column-title">{marketType === 'houses' ? 'HOUSES' : 'SPORTS'} DRAWN</h3>
                  <div className="character-markets-list">
                ...
                {/* HOUSES/SPORTS NOT DRAWN Column */}
                <div className="character-market-column">
                  <h3 className="character-column-title">{marketType === 'houses' ? 'HOUSES' : 'SPORTS'} NOT DRAWN</h3>
                  <div className="character-markets-list">
                    {crewMarkets.map(crewMarket => {
